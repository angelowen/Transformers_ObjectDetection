...
from datasets.word import make_word_transforms
...
def get_args_parser():
    ...
    # dataset parameters
    parser.add_argument('--dataset_file', default='word')
    parser.add_argument('--data_path', type=str)
    parser.add_argument('--data_panoptic_path', type=str)
    parser.add_argument('--remove_difficult', action='store_true')

    parser.add_argument('--output_dir', default='./output/test_result/',
...
def infer(images_path, model, postprocessors, device, output_path):
    model.eval()
    duration = 0
    for img_sample in sorted(images_path):
        filename = os.path.basename(img_sample)
        print("processing...{}".format(filename))
        orig_image = Image.open(img_sample)
        w, h = orig_image.size
        transform = make_word_transforms("val")
        dummy_target = {
        ...
        if len(bboxes_scaled) == 0:
            print(f"{filename} is skiped because of bboxes_scaled equal to zero")
            continue
